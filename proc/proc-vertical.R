...
vars_ver <- c("ros4", "b21a", "b21", "b18", "b31", "b13", "pn4", "eff1",
              "eff2", "exc7", "b12", "b32", "b47a", "vb21n", "b1",
              "b2", "b3", "ing4", "vb2", "pol1", "cp6", "cp7", "cp8", "cp13", "prot3" )

vars <- c("wave","year","pais","wt", vars_ver)
...
vars_mod <- names(prom_wave)[c(3:28)]
...
# Se almacena la data.
write_rds(prom_wave,"input/data/lapop_proc_vertical.RDS")
...
data <- WVS %>% select(COW_NUM, S020,S017,
                       E069_06,
                       E069_17,
                       E069_11,
                       E069_12,
...
data <- data %>%
  setNames(c("pais","wave","wt",
             "confianza_policia",
             "confianza_sist_judicial",
             "confianza_gobierno",
             "confianza_part_politicos",
...
data$igualdad_ingresos[data$igualdad_ingresos<1] <- NA
data$igualdad_ingresos <- 11-data$igualdad_ingresos
...
# 
data$firma_peticiones[data$firma_peticiones<1] <- NA
data$firma_peticiones <- 4-data$firma_peticiones
# 
# 
# data$boycotts[data$boycotts<1] <- NA
# data$boycotts <- 4-data$boycotts
# 
data$protestas_pacificas[data$protestas_pacificas<1] <- NA
data$protestas_pacificas <- 4-data$protestas_pacificas

data$participacion_huelgas[data$participacion_huelgas<1] <- NA
data$participacion_huelgas <- 4-data$participacion_huelgas
# 
data$trabajo_duro[data$trabajo_duro<1] <- NA
data$trabajo_duro <- 11-data$trabajo_duro
...
data$confianza_policia[data$confianza_policia<1] <- NA
data$confianza_policia <- 5-data$confianza_policia

data$confianza_sist_judicial[data$confianza_sist_judicial<1] <- NA
data$confianza_sist_judicial <- 5-data$confianza_sist_judicial

data$confianza_gobierno[data$confianza_gobierno<1] <- NA
data$confianza_gobierno <- 5-data$confianza_gobierno

data$confianza_part_politicos[data$confianza_part_politicos<1] <- NA
data$confianza_part_politicos <- 5-data$confianza_part_politicos

data$confianza_parlamento[data$confianza_parlamento<1] <- NA
data$confianza_parlamento <- 5-data$confianza_parlamento

data$importancia_democracia[data$importancia_democracia<1] <- NA
...
write_rds(prom_wave,"input/data/wvs_proc_vertical.RDS")

# Latinobarometro ----


rm(list=setdiff(ls(),c("prop_casos_perdidos",
...
variables <- rbind(redistribucion)

# Carga de archivos
dta <- list.files(path = "input/data/Latinobarometro/", pattern = ".sav")
dta <- paste0(file = "input/data/Latinobarometro/",dta)
...
vars_mod <- names(prom_wave_latin)[c(4)]
...
write_rds(prom_wave_latin,"input/data/lb_proc_vertical.RDS")
...
data_lapop <- readRDS("input/data/lapop_proc_vertical.rds")
data_latinobarometro <- readRDS("input/data/lb_proc_vertical.RDS")
data_wvs <- readRDS("input/data/wvs_proc_vertical.rds")
...
datos <- merge(datos, data_latinobarometro, by=c("pais", "wave"), all=TRUE)
...
save(datos, file="input/data/datos_vertical.RData")
