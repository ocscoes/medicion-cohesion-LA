...
LAPOP incluye dos indicadores que miden orgullo sobre el sistema político y necesidad de apoyar el sistema político del país. Por su lado, WVS incluye preguntas sobre orgullo de pertenecer al país y sobre la posibilidad de luchar en una guerra por el país. La @tbl-descriptivos-pertenencia muestra los principales estadísticos descriptivos de estas variables.

```{r}
#| label: tbl-descriptivos-pertenencia
...
descriptivos_pertenencia <- datos %>%
  select(c("b4", "b6", "voluntad_luchar_pais", "orgullonacional")) %>%
  setNames(c("Orgullo Sist. Político",
             "Deber de apoyar Sist. Político",
             "Voluntad de Luchar por el País", "Orgullo Nacional"))

print(dfSummary(descriptivos_pertenencia,
          headings=FALSE,
          style = "multiline",
          plain.ascii = FALSE,
          tmp.img.dir = 'tmp',
...
```{r}
#| label: fig-correlacion-pertenencia
...
La @fig-correlacion-pertenencia muestra la correlación entre los indicadores de la subdimensión Sentido de Pertenencia. Se muestra una correlación muy alta y significativa entre Orgullo con el sistema político y el deber de apoyar el sistema político (r=0.9; p<0.001) y una correlación alta entre la Voluntad de luchar por el país y el orgullo nacional (r=0.64; p<0.001). El resto de las correlaciones son bajas, negativas o incluso negativas.

Por criterios estadísticos y teóricos, se dejará una única variable: **Orgullo Nacional** (WVS). 

## Calidad de vida en el vecindario

LAPOP incluye varios indicadores de calidad de vida urbana (principalmente, acceso a servicios básicos), así como indicadores subjetivos y objetivos de calidad. Por otro lado, la WVS incluye un indicador de confianza en los vecinos. La @tbl-descriptivos-vecindario muestra los principales descriptivos de esta subdimensión.

```{r}
#| label: tbl-descriptivos-vecindario
...
descriptivos_vecindario <- datos %>%
  select(c("aoj11", "vic1ext",
              "sd2new2",
              "sd3new2",
           "sd5new2",
              "sd6new2",
           "confiar_vecinos")) %>%
  setNames(c("Percepción seguridad",
             "Victima de delitos",
             "Satisfacción vías",
             "Satisfacción escuelas",
             "Satisfacción servicios sanitarios",
...
print(dfSummary(descriptivos_vecindario,
          headings=FALSE,
          style = "multiline",
          plain.ascii = FALSE,
          tmp.img.dir = 'tmp',
...
```{r}
#| label: fig-correlación-vecindario
...
La @fig-correlación-vecindario muestra la asociación entre todas las variables incluidas inicialmente en esta subdimensión. Tomando esta información como referencia, se estimarán promedios simples entre los indicadores con las correlaciones más altas, dejando tres variables: (a) Servicios básicos; (b) Seguridad urbana; y (c) Confianza en vecinos.

### Correlaciones variables promediadas
...
```{r}
#| label: fig-correlación-vecindario-subdimensiones
...
La @fig-correlación-vecindario-subdimensiones muestra la asociación entre estas tres variables construidas. Por un lado, la asociación entre servicios básicos y las otras dos variables van de muy baja a moderada. Por otro lado, la asociación entre seguridad urbana y confianza en vecinos es alta y significativa (r=0.49; p<0.001).

Con esta información disponible, se eliminarán las variables de acceso a servicios básicos, dejando solo aquellas relacionadas con seguridad urbana y confianza en vecinos.

## Redes

LAPOP incluye preguntas de participación cívica en organizaciones civiles, tales como organizaciones religiosas, escolares y vecinales, además de un indicador de confianza interpersonal. Por su lado, la WVS incluye indicadores de comportamiento "antisocial", que fueron invertidas para medir comportamiento cívico o prosocial. La @tbl-descriptivos-redes muestra los principales estadísticos descriptivos de esta subdimensión.

```{r}
#| label: tbl-descriptivos-redes
...
descriptivos_redes <- datos %>%
  select(c("it1", "cp6", "cp7", "cp8",
           "aceptar_soborno", "evadir_impuestos", "recibir_beneficios", "evadir_transporte"
           )) %>%
...
print(dfSummary(descriptivos_redes,
          headings=FALSE,
          style = "multiline",
          plain.ascii = FALSE,
          tmp.img.dir = 'tmp',
...
La @fig-correlacion-redes muestra las correlaciones entre todos los indicadores disponibles. Para simplificar la información, se estimaron promedios simples entre las variables con las correlaciones más altas.
...
La @fig-correlacion-redes-2 incluye las correlaciones entre estas variables construidas. Por un lado, la correlación entre sociabilidad y las otras dos variables es alta, pero son negativas. Por otro lado, la correlación entre confianza interpersonal y conducta prosocial es alta y significativa (r=0.61; p<0.01).

Por lo tanto, se eliminarán los indiciadores de sociabilidad, pues la correlación negativa dificultaría la interpretación sustantiva del indice.
...
```{r}
#| label: fig-correlaciones-propuesta-a
...
En general, en la @fig-correlaciones-propuesta-a se observan correlaciones sobre 0,3 y 0,5. Sin embargo, el indicador de orgullo nacional muestra correlaciones más inconsistentes con el resto de los indicadores

```{r}
kmo_result <- KMO(subset_a)
...
bartlett_result <- cortest.bartlett(subset_a)
...
Tanto el KMO (0,7) como el test de esfericidad de Bartlett ($\chi^2$= 1489,5; p<0,001, df=28) indican que los datos son adecuados para realizar un análisis factorial.
...
sjPlot::tab_fa(subset_a, method = "ml", rotation="promax", show.comm = TRUE)
```

La varianza explicada por los dos factores es de un 66%. El indicador de orgullo nacional no se anida bien en ningún factor. El RMSEA (0,273) sugiere un mal ajuste del modelo. 

```{r echo=F}
ma_3f <- fa(subset_a, nfactors = 3, 
   fm = "ml", rotate = "none")

sjPlot::tab_fa(subset_a, nmbr.fctr = 3, method = "ml", rotation="promax", show.comm = TRUE)
```

La varianza explicada sube a un 77% y orgullo nacional ahora sí anida dentro de un factor. Sin embargo, los índices de ajuste no mejoran (RMSEA= 0,276)

```{r echo=F}
ma_4f <- fa(subset_a, nfactors = 4, 
   fm = "ml", rotate = "none")
# Mejor ajuste estadístico. Voy a rotarlo para generar una solución clara. Oblimin, asumiendo correlación entre factores. 

sjPlot::tab_fa(subset_a, nmbr.fctr = 4, method = "ml", rotation="promax", show.comm = TRUE)
```
...
sjPlot::tab_fa(subset_a, nmbr.fctr = 4, method = "ml", rotation="oblimin", show.comm = TRUE)
```
...
Por último, se hizo un ejercicio similar pero incluyendo únicamente indicadores incluidos en LAPOP. El resultado de esto, es que dos subdimensiones (orgullo nacional y comportamiento prosocial) quedan fuera del análisis. Sin embargo, la correlación entre las dos subdimensiones restantes es buena (0,53). 
...
bartlett_result <- cortest.bartlett(subset_b)
...
mb_4f <- fa(subset_b, nfactors = 4,
   fm = "ml", rotate = "none")
sjPlot::tab_fa(subset_b, nmbr.fctr = 4, method = "ml", show.comm = TRUE)
# Mejor ajuste estadístico. Voy a rotarlo para generar una solución clara. Oblimin, asumiendo correlación entre factores. 
...
sjPlot::tab_fa(subset_b, nmbr.fctr = 4, method = "ml", rotation = "oblimin", show.comm = TRUE)
...
Finalmente, considerando los análisis previos, proponemos utilizar dos subdimensiones para medir cohesión horizontal:

- Confianza interpersonal: Medida a partir del indicador de confianza en terceros

- Seguridad: Medida a partir de las variables de seguridad objetiva (victima de delitos) y seguridad subjetiva (percepción de seguridad)
